...
public class Mouse {
    ...
    public void moveRelMouse(Point point){
        try {
            point.x = point.x - GlazMain.window.getPos().x;
            point.y = point.y - GlazMain.window.getPos().y;
            Robot moveRobot = new Robot();
            moveRobot.mouseMove(point.x, point.y);
        } catch (AWTException e) {
            e.printStackTrace();
    ...
    public Point getRelPosition(){
        PointerInfo a = MouseInfo.getPointerInfo();
        Point b = a.getLocation();
        b.x = b.x - GlazMain.window.getPos().x;
        b.y = b.y - GlazMain.window.getPos().y;
        return b;
    }
    ...
    public void moveRelPosition(Point pos){
        ...
        try {
            Robot moveRobot = new Robot();
            pos.x = pos.x - GlazMain.window.getPos().x;
            pos.y = pos.y - GlazMain.window.getPos().y;
